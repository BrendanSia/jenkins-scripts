def call(String status, String buildNumber, String buildDate, String failedStage = null, String chatId, String botToken) {
    script {
        if (status == 'SUCCESS') {
            sh """
                sh src/scripts/success_notification.sh "${buildNumber}" "${buildDate}" "${chatId}" "${botToken}"
            """
        } else if (status == 'FAILURE') {
            sh """
                sh src/scripts/failure_notification.sh "${buildNumber}" "${buildDate}" "${failedStage}" "${chatId}" "${botToken}"
            """
        } else {
